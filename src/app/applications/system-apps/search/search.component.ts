...
import { Component, OnInit, AfterViewInit, OnDestroy, Renderer2, ElementRef, ViewChild, HostBinding } from '@angular/core';
...
import { FileIndexIDs, ActivityType } from 'src/app/system-files/commons/common.enums';
...
export class SearchComponent implements OnInit, AfterViewInit, OnDestroy {
   ...
   * `.solid-surface` class, which swaps the translucent/blurred search panels
   * for opaque ones. Default setting is OFF, so this starts true.
   */
  @HostBinding('class.solid-surface') isSolidSurface = false;

  /**
   * Exposes the search surface color through the `--surface-color` custom
  ...
  /**
   * Reads the persisted "Transparency effects" setting and keeps the search
  ...
  /**
   * Computes the search surface color: the accent color when the "Show accent
   * color on Start and taskbar" toggle is ON, otherwise the default system
   * color. The accent color always has a default, so it's used directly.
   */
  ...
  async handleBestMatchHightLight(toggle:boolean): Promise<void>{
    ...
    if(bestMatchElmnt){
      const accentColor = this._defaultService.getDefaultSetting(Constants.DEFAULT_ACCENT_COLOR);
      bestMatchElmnt.style.backgroundColor = (toggle) ? accentColor : Constants.EMPTY_STRING;
    }
  }
