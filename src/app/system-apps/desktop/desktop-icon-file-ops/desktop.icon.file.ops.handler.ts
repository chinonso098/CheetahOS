...
export interface DesktopIconFileOpsHandlerInit {
    runApplication: (file: FileInfo) => Promise<void>;
    /**
...
export class DesktopIconFileOpsHandler {
    ...
    sourceData: GeneralMenu[] = [
        { icon: '', label: 'Open', action: () => this.onTriggerRunApplication() },
        { icon: `${Constants.IMAGE_BASE_PATH}recycle bin_folder_small.png`, label: 'Empty Recycle Bin', action: () => this.onEmptyRecycleBin() },
        { icon: '', label: 'Pin to Quick access', action: () => this.doNothing() },
        { icon: '', label: 'Open in Terminal', action: () => this.openInTerminal() },
        { icon: '', label: 'Pin to Start', action: () => this.doNothing() },
        { icon: '', label: 'Pin to Taskbar', action: () => this.pinIconToTaskBar() },
        { icon: '', label: 'Cut', action: () => this.onCut() },
        { icon: '', label: 'Copy', action: () => this.onCopy() },
        { icon: '', label: 'Create shortcut', action: () => this.createShortCut() },
        { icon: '', label: 'Delete', action: () => this.onDelete() },
        { icon: '', label: 'Rename', action: () => this.onRenameFileTxtBoxShow() },
        { icon: '', label: 'Properties', action: () => this.showPropertiesWindow() },
        { icon: '', label: 'Confirm Delete', action: () => this.onConfirmDelete() },
        { icon: '', label: 'Recycle on Delete', action: () => this.onDeleteMoveToRecycleBin() },
    ];
    ...
    /**
     * Re-entry counter for the rename form's `(ngSubmit)`. If the
     * form is pristine and submit fires twice in a row, that's
     * treated as the "user wants to cancel" gesture and the textbox
     * is hidden. Reset to 0 inside `onRenameFileTxtBoxDataSave`.
     */
    ...
    onTriggerRunApplication(): void {
        this._callbacks.runApplication(this.selectedFile);
    }
    ...
    async openInTerminal(): Promise<void> {
        ...
        this._callbacks.runApplication(selectedFile);
    }
    ...
    async createShortCut(): Promise<void> {
        ...
        const result = await this._fileService.writeFileAsync(Constants.DESKTOP_PATH, shortCut);
        if (result) {
...
`;
     ...
     * → commit (save). Pristine form, fired more than once →
     * treat as cancel and hide the textbox. The double-submit
     * counter resets to 0 inside `onRenameFileTxtBoxDataSave`.
     *
