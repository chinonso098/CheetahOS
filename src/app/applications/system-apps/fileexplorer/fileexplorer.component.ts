...
import { AfterViewInit, Component, OnInit, OnDestroy, ViewChild, ViewChildren, QueryList, ElementRef, ViewEncapsulation, Input, HostBinding} from '@angular/core';
...
import { GeneralMenu, NestedMenu, MenuPosition, NestedMenuItem } from 'src/app/shared/system-ui-components/menu/menu.types';
...
export class FileExplorerComponent implements BaseComponent, OnInit, AfterViewInit, OnDestroy {
  ...
  // (Option 1 — tooltip escapes the window) Typed ref to the file-info tooltip
  // element. We relocate this node to `document.body` in ngAfterViewInit so it
  // can render OUTSIDE the file-explorer window (every ancestor up to
  // `.window-container` uses overflow:hidden + transform, which would otherwise
  // clip it). static:false because the node is bound/visible only on hover.
  @ViewChild('infoTipContainer', {static: false}) infoTipContainer?: ElementRef<HTMLElement>;
  // The two context menus are relocated to `document.body` on open for the
  // same reason as the tooltip above (escape the window's overflow:hidden +
  // transform clipping). Unlike the always-present tooltip, these are *ngIf
  // nodes recreated on every open, so we re-relocate each time they show.
  // `read: ElementRef` is required because the refs point at `cos-menu`
  // components — without it Angular hands back the component instance (which
  // has no `nativeElement`) and the relocation silently no-ops.
  ...
  /**
   * Drives the highlight-default tab background from the global personalization
   * accent. There is always a default accent color (never None), so this inline
   * custom property always carries the current accent and feeds the tab's
   * `background-color: var(--fx-tab-accent, var(--fx-accent))`. Deliberately a
   * dedicated token (not --fx-accent) so ONLY that one tab rule is affected.
   */
  @HostBinding('style.--fx-tab-accent') tabAccentColor = Constants.EMPTY_STRING;

  // Mirror the dedicated title-bar/border transparency + title-bar accent toggles
  // onto the host so the header surfaces (tab strip, nav container, spacer) can
  // frost/tint to match the window title bar. Driven by
  // DEFAULT_SHOW_TRANSPARENCY_EFFECT_ON_TITLE_BAR_AND_WINDOW_BORDER and
  // DEFAULT_SHOW_ACCENT_COLOR_TITLE_BARS_AND_WINDOW_BORDERS.
  @HostBinding('class.fx-transparent') isTransparencyOn = false;
  @HostBinding('class.fx-titlebar-accent') isTitleBarAccentOn = false;
  ...
  constructor(processIdService:ProcessIDService, runningProcessService:RunningProcessService, fileService:FileService, 
    ...
    this._hideContextMenuSub = this._menuService.closeContextMenu.subscribe((ownerUId) => {
      if(ownerUId === uId) // targeted: only close when this instance owns the open menu
  ...
  ngOnInit():void{
    ...
    this.applyTabAccentColor();
    this.applyHeaderSurfaceState();
    this._accentChangeSub = this._defaultService.defaultSettingsChangeNotify.subscribe((key:string) => {
      ...
      if(key === Constants.DEFAULT_SHOW_TRANSPARENCY_EFFECT_ON_TITLE_BAR_AND_WINDOW_BORDER
        || key === Constants.DEFAULT_SHOW_ACCENT_COLOR_TITLE_BARS_AND_WINDOW_BORDERS){
        this.applyHeaderSurfaceState();
      }
    });
  ...
  async ngAfterViewInit():Promise<void>{
    ...
    // (Option 1) Lift the info tooltip out of the window subtree so it can
    // overflow the window edges instead of being clipped / spawning a scrollbar.
  ...
  ngOnDestroy(): void {
    ...
    // (Option 1) Remove the body-level tooltip node we relocated in
    // ngAfterViewInit. Guarded so it's a no-op if Angular's view teardown
    // already detached it (prevents a double-remove).
  ...
  /**
   * Drives the highlight-default tab background from the global personalization
   * accent. There is always a default accent color (it's never None), so the
   * tab simply mirrors whatever the current accent is.
   */
  private applyTabAccentColor():void{
    this.tabAccentColor = this._defaultService.getDefaultSetting(Constants.DEFAULT_ACCENT_COLOR);
  }

  /**
   * Reads the global transparency + title-bar accent toggles and mirrors them as
   * host classes (.fx-transparent / .fx-titlebar-accent) so the header CSS can
   * frost/tint the top surfaces to match the window title bar.
   */
  private applyHeaderSurfaceState():void{
    this.isTransparencyOn = this._defaultService.getDefaultSetting(Constants.DEFAULT_SHOW_TRANSPARENCY_EFFECT_ON_TITLE_BAR_AND_WINDOW_BORDER) === Constants.TRUE;
    this.isTitleBarAccentOn = this._defaultService.getDefaultSetting(Constants.DEFAULT_SHOW_ACCENT_COLOR_TITLE_BARS_AND_WINDOW_BORDERS) === Constants.TRUE;
  }
  ...
  setNavButtonsColor():void{
    ...
    const activeFill = 'var(--fx-text)';
    const inactiveFill = 'var(--fx-text-dim)';
  ...
  private rebuildUpStackFromCurrent(): void {
    ...
    this.upNavBtnStyle = { fill: this.isUpBtnActive ? 'var(--fx-text)' : 'var(--fx-text-dim)' };
  }
  ...
  private async navigateTo(targetPath: string, kind: string): Promise<void>{
    ...
    this.prevNavBtnStyle = { fill: this.isPrevBtnActive ? 'var(--fx-text)' : 'var(--fx-text-dim)' };

    this.isNextBtnActive = this.nextPathEntries.length > 0;
    this.nextNavBtnStyle = { fill: this.isNextBtnActive ? 'var(--fx-text)' : 'var(--fx-text-dim)' };
    ...
    await this.loadFiles();
    // Clear any selection/hover/tooltip carried over from the prior folder —
    // it's keyed by list index and would otherwise linger in the new listing.
  ...
  */
  ...
  onShowIconContextMenu(evt:MouseEvent, file:FileInfo, id:number):void{
    ...
    // it is relocated to document.body and positioned `fixed` below. The helper
    // computes positions as `evt.clientX - rect.left`, so a viewport-origin rect
    // yields viewport-relative coordinates directly.
  ...
  adjustIconContextMenuData(file:FileInfo):void{
    ...
    this.sourceData.forEach(row => { if (row.label === 'Open') row.icon = openImg; });
  ...
  /**
   * Clear all per-listing selection/hover/tooltip state after navigating to a
   * new directory. Selection is keyed by *ngFor index, so a stale
   * `selectedElementId` / `hoveredElementId` / `markedBtnIds` from the prior
   * folder would otherwise keep painting an icon at that same index in the new
   * listing (the "lingering highlight" seen when drilling down quickly). The
   * file-info tooltip is also dismissed, since it points at a file that is no
   * longer shown. Call this AFTER `loadFiles()` so the reset isn't undone by
   * any selection work that ran before the listing swapped.
   */
  ...
  private async showFileExplorerToolTip(evt:MouseEvent, file: FileInfo): Promise<void> {
    ...
    requestAnimationFrame(() => {
      // The tooltip now lives on `document.body` with `position: fixed`, so its
      // coordinates are in viewport space — we place it directly at the cursor
      // (no offsetParent math needed). We then clamp/flip it so it never leaves
      // the viewport, even when the file-explorer window is near a screen edge.
      const tip = this.infoTipContainer?.nativeElement;
      ...
      // the fully-rendered tip (its *ngFor rows are in the DOM by now).
  ...
  /**
   * (Option 1) Move the info tooltip element out of the file-explorer window
   * and into `document.body` so it can extend beyond the window's edges.
   *
   * Why this is necessary: every ancestor up to `.window-container` uses
   * `overflow: hidden`, and the window animates via `transform` / `will-change`.
   * That means a descendant tooltip is always clipped, and even a
   * `position: fixed` child would anchor to the transformed window rather than
   * the viewport. Lifting the node to the body level is the only reliable fix.
   *
   * Angular keeps full ownership of the node after the move: the show/hide
   * class, the left/top bindings and the *ngFor rows all keep updating because
   * Angular addresses the element by reference, not by DOM location. The
   * component-scoped CSS still applies too, since the element keeps its
   * `_ngcontent` attribute. The only thing it loses is the `--fx-*` theme
   * variables (declared on `:host`), so we copy the few it uses onto the node.
   */
  private relocateInfoTipToBody(): void {
    const tip = this.infoTipContainer?.nativeElement;
    if(!tip) return;

    // Re-apply the theme variables the tooltip depends on, reading their
    // resolved values from a node still inside the host (custom properties no
    // longer inherit once the tooltip leaves the host subtree).
    const hostStyles = getComputedStyle(this.fileExplorerRootContainer.nativeElement);
    for(const varName of ['--fx-bg', '--fx-border', '--fx-text']){
      tip.style.setProperty(varName, hostStyles.getPropertyValue(varName).trim());
    }
  ...
  /**
   * Move a context-menu element into `document.body` so it can extend past the
   * file-explorer window's clipped edges, exactly like the tooltip above.
   *
   * Difference from the tooltip: the menus are `*ngIf` nodes, recreated on
   * every open, so this runs each time a menu is shown (scheduled via a 0ms
   * timeout from the show handlers, after Angular has rendered the node). The
   * menu's CSS is self-contained (it carries no `--fx-*` dependency — it is the
   * same `cos-menu` used at desktop/body level), so no theme variables need to
   * be copied across. Angular still owns the node by reference: the `[style]`
   * binding keeps updating and `*ngIf` removal still tears it down correctly
   * even though it now lives under `document.body`.
   */
