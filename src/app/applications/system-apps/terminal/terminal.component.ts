...
import { Component, ElementRef, ViewChild, OnInit, AfterViewInit, OnDestroy, Input, HostBinding } from '@angular/core';
...
export class TerminalComponent implements BaseComponent, OnInit, AfterViewInit, OnDestroy{
  ...
  // Tracks the system light/dark theme so the terminal can recolor. Bound to
  // the host as `theme-light`; the existing dark styling is the CSS default.
  ...
  echoCommands:string[] = ["close", "curl","date", "echo", "help", "hostname", "list", "open", "sysmetric", "sysreset", "sysrestart", "syssdwn", "version", "whoami", "weather","pwd"];
  ...
  constructor( processIdService:ProcessIDService,runningProcessService:RunningProcessService, processHandlerService:ProcessHandlerService, fileService:FileService,  formBuilder:FormBuilder,
    ...
    this._terminaCommandsProc = new TerminalCommandProcessor(processHandlerService, runningProcessService, fileService, activityHistoryService, systemMetric, systemNotificationService, windowService, defaultService, sessionManagementService);
  ...
  async ngAfterViewInit():Promise<void>{
    ...
    if(this._fileInfo 
      && this._fileInfo.getCurrentPath !== Constants.EMPTY_STRING
      && !this._fileInfo.getCurrentPath.includes(Constants.URL)){
