...
export class TaskbarMenuHandler {
    ...
    /** Pixel offset for app-icon menu when no app instances are running. */
    private static readonly APP_ICON_MENU_X_OFFSET_PX = -60;
    ...
    openAppIconMenu(rect: DOMRect, tskBarIcon: TaskBarIconInfo): void {
        ...
        if (processCount === 0) {
            this.appIconMenuStyle = {
                'position': 'absolute',
                'transform': `translate(${String(rect.x + TaskbarMenuHandler.APP_ICON_MENU_X_OFFSET_PX)}px, ${String(rect.y + TaskbarMenuHandler.APP_ICON_MENU_Y_OFFSET_NO_INSTANCES_PX)}px)`,
                'z-index': 5,
            };
        } else {
            this.appIconMenuStyle = {
                'position': 'absolute',
                'transform': `translate(${String(rect.x + TaskbarMenuHandler.APP_ICON_MENU_X_OFFSET_PX)}px, ${String(rect.y + TaskbarMenuHandler.APP_ICON_MENU_Y_OFFSET_WITH_INSTANCES_PX)}px)`,
                'z-index': 5,
            };
        }
    }
    ...
    initContextMenuData(taskManagerLauncher: () => void): void {
        ...
        this.contextMenuData = [
            ...
            { icon: empty, label: 'Task Manager', action: () => this._taskManagerLauncher() },
