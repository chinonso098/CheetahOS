...
import {Component, Input, OnChanges, SimpleChanges, AfterViewInit, EventEmitter, Output, OnDestroy, HostBinding} from '@angular/core';
...
import { OpensWith, AppOpeningSelection, InformationUpdate } from 'src/app/system-files/commons/common.interfaces';
import { FileInfo } from 'src/app/system-files/fs/file.info';
import { AppDirectory } from 'src/app/system-files/app.directory';

/** A selectable app entry for the Create Shortcut dialog's Browse dropdown. */
interface DialogAppEntry {
  name: string;
  icon: string;
}
...
export class DialogComponent implements BaseComponent, OnChanges, AfterViewInit, OnDestroy {
  ...
  @Input() AppSelection!:OpensWith; 

  // The following outputs are used to communicate user actions back to the parent component or service.
  @Output() confirm = new EventEmitter<void>();
  @Output() cancel = new EventEmitter<void>();
  @Output() ok = new EventEmitter<string>();

  // Tracks the system dark/light theme so the dialog body can recolor. Bound to the
  // host as `theme-dark`; the existing light styling remains the CSS default.
  @HostBinding('class.theme-dark') isDarkTheme = false;
  private _themeChangeSub!:Subscription;

  // Drives `--st-accent` from the global personalization accent so accent-aware
  // dialog elements (e.g. the power dialog wordmark/icon) recolor to the chosen accent.
  ...
  applicationSelectionNotification = UserNotificationType.AppSelection;
  ...
  createShortcutNotification =  UserNotificationType.CreateShortcut;
  ...
  applicationOptions:AppOpeningSelection[] = [];
  applicationExt:string = Constants.EMPTY_STRING;
  selectedAppName:string = Constants.EMPTY_STRING;
  alwaysUseSelectedApp = false;
  showAnotherAppText = false;

  // Create-shortcut dialog: app-picker state. `shortcutAppList` is the catalogue
  // shown under the Browse button; selecting an app fills the field and enables Next.
  shortcutAppList:DialogAppEntry[] = [];
  isShortcutDropdownOpen = false;
  shortcutSelectedApp = Constants.EMPTY_STRING;
  ...
  readonly SET_PWR_OR_APP_SELECTION_DIALOG_PID_ON_CLOSE  = 'Update0';
  ...
  shortCutLocation = Constants.EMPTY_STRING;
  ...
  constructor(runningProcessService:RunningProcessService, notificationServices:UserNotificationService,  windowService:WindowService,
              ...
              audioService:AudioService, fileService:FileService, defaultService:DefaultService, themeService:ThemeService){
  ...
  ngOnChanges(changes: SimpleChanges):void{
    ...
    if(this.notificationType === UserNotificationType.AppSelection){ 
      if(!this.AppSelection || !this.AppSelection.apps || this.AppSelection.apps.length === 0) return;

      this._systemNotificationService.setAppSelectionDialogPid(this.processId);

      this.applicationExt = this.AppSelection.fileType;

      this.AppSelection.apps.forEach(app => {
        app.appIcon = this._fileService.getAppAssociaton(app.appName);
        // app.appIcon = `${Constants.IMAGE_BASE_PATH}${app.appIcon}`;
      });
      this.applicationOptions = this.AppSelection.apps;

      const defaultApp = this.applicationOptions.find(x => x.isDefault);
      this.selectedAppName = defaultApp ? defaultApp.appName : Constants.EMPTY_STRING;
    }

    if(this.notificationType === UserNotificationType.CreateShortcut){ 
  
      // Reset the picker and load the selectable app catalogue (excluding hidden
      // apps like cheetah/clippy/runsystem/clipboard) for the Browse dropdown.
      this.shortcutSelectedApp = Constants.EMPTY_STRING;
      this.isShortcutDropdownOpen = false;
      this.shortcutAppList = this._appDirectory.getAppList()
        .filter(appName => !this._appDirectory.getHiddenApp().includes(appName))
        .map(appName => ({ name: appName, icon: this._fileService.getAppAssociaton(appName) }));
    }
  }

  /** Sets `--st-accent` to the chosen personalization accent (always has a default). */
  ...
  async ngAfterViewInit(): Promise<void> {
    const notificationAudioDelay = 200; //200ms
    await CommonFunctions.sleep(notificationAudioDelay);
  ...
  onYesDialogBox():void{
    ...
    if(this.notificationOption === UserNotificationType.AppSelection){
      if(this.alwaysUseSelectedApp)
        this._processHandlerService.setDefaultAppOverRide(`${Constants.DOT}${this.applicationExt}`, this.selectedAppName);
        
      this.ok.emit(this.selectedAppName);
    }

    // if(this.notificationOption === UserNotificationType.ZipExtract){
    //   this.ok.emit(this.zipExtractDestination);
    // }
  ...
  */
  ...
  async onYesPowerDialogBox(): Promise<void>{
    const notificationEventDelay = 200; //200ms    
    ...
    await CommonFunctions.sleep(notificationEventDelay);
  ...
  onCloseDialogBox(evt?:MouseEvent):void{
    ...
    if(this.notificationOption === UserNotificationType.PowerOnOff){
      this.setPwrDialogPid(this.SET_PWR_OR_APP_SELECTION_DIALOG_PID_ON_CLOSE);
    }
  ...
  onAppOptionSelect(appName:string, evt?:MouseEvent):void{
    evt?.stopPropagation();
    this.selectedAppName = appName;
  }

  onLookForAnotherApp():void{
    this.showAnotherAppText = true;
  }

  /** Toggle the Create Shortcut app picker open/closed. */
  onShortcutBrowse(evt:MouseEvent):void{
    evt.stopPropagation();
    this.isShortcutDropdownOpen = !this.isShortcutDropdownOpen;
  }

  /** Pick an app: fill the field, close the list, and enable Next. */
  onSelectShortcutApp(app:DialogAppEntry, evt:MouseEvent):void{
    evt.stopPropagation();
    this.shortcutSelectedApp = app.name;
    this.isShortcutDropdownOpen = false;
  }

  onNext():void{
    if(this.shortcutSelectedApp){
      this.ok.emit(this.shortcutSelectedApp);
    }

   this._userNotificationServices.closeDialogMsgBox(this.processId);
  }

  /** Back arrow: reset the picker (clear the field, disable Next, close the list). */
  onShortcutBack():void{
    this.shortcutSelectedApp = Constants.EMPTY_STRING;
    this.isShortcutDropdownOpen = false;
  }
  ...
  showEstimating():void{
    const showEstimatingDelay = 250; //.25 sec
