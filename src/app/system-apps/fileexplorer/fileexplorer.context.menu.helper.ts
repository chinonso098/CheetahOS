...
export namespace FileExplorerContextMenuHelper {
    ...
    export const adjustIconContextMenuData = (file:FileInfo, sourceData:GeneralMenu[], isRecycleBinFolder:boolean):[GeneralMenu[], string] => {
        const menuData:GeneralMenu[] = [];
        let menuOrder = Constants.EMPTY_STRING;
        const isZipFile = (file.getFileExtension === '.zip');

        if(file.getIsFile){
            if(EDIT_NOT_ALLOWED.includes(file.getCurrentPath.replace(Constants.ROOT, Constants.EMPTY_STRING))){
                menuOrder = Constants.FILE_EXPLORER_UNIQUE_MENU_ORDER;
                for(const x of sourceData){
                    if(x.label === 'Cut' || x.label === 'Delete' || x.label === 'Rename' || x.label === 'Extract All...') continue;
                    else
                        menuData.push(x);
                }
            }else if(isRecycleBinFolder){
                menuOrder = Constants.FILE_EXPLORER_RECYCLE_BIN_MENU_ORDER;
                for(const x of sourceData){
                ...
                menuOrder = Constants.FILE_EXPLORER_FILE_MENU_ORDER;
                for(const x of sourceData){
            ...
            if(isRecycleBinFolder){
                ...
                menuOrder = Constants.FILE_EXPLORER_FOLDER_MENU_ORDER;
                menuData.push(...sourceData.filter(x => x.label !== 'Restore' && x.label !== 'Extract All...'));
            }
        }
